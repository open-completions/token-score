from token_score import (
    ...
    compute_document_parser_result,
)


def test_compute_document_parser_result_go():
    document = Document(
        lang="go",
        content=b'package hello_world\n\n// Hello World\nfunc main() {\na := "Hello World"}\n',
    )

    assert compute_document_parser_result(
        document.content, PARSERS[document.lang]
    ) == DocumentParserResult(
        semantic_tokens=[
...
def test_compute_document_parser_result_python():
    document = Document(
...
def test_compute_document_parser_result_java():
    document = Document(
        lang="java",
...
def test_compute_document_parser_result_javascript():
    document = Document(
        lang="javascript",
...
def test_compute_document_parser_result_cpp():
    document = Document(
        lang="c++",
